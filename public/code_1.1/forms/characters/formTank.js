class formTank {
    static initialize() {
        ...
        // buy button
        this.charPrice = characterTank.getPrice();
        this.userMoney = Number(localStorage.plinMoney);
        this.charUnlocked = Boolean(localStorage.unlTank);
    ...
    static render() {
        ...
        text("Tank", width / 2, defaultTitleY);

        // rendering money
        objectGui.renderUserMoney();

        // rendering char stats
        var stats = characterTank.getStats();
        noStroke();
        fill(255);
        textAlign(RIGHT);
        ...
        var abilities = characterTank.getAbilities();
        noStroke();
        fill(255);
        textAlign(LEFT);
        textSize(this.statsSize);
    ...
    static mousePressed() {
        ...
        if (this.buttonBuy.hovered) {
            audioClick.play();
            if (Number(localStorage.plinMoney) >= characterTank.getPrice()) {
                // unlocking char
                localStorage.unlTank = true;
                this.buttonBuy.locked = true;
                localStorage.plinMoney = Number(localStorage.plinMoney) - characterTank.getPrice();
